...
(function() {
  ...
  function updateStarredNav() {
    ...
    // Show/hide the entire badge based on starred count
    badge.style.display = starred.length > 0 ? '' : 'none';
  ...
  window.searchListing = function(input) {
    ...
    requestAnimationFrame(function() {
      document.querySelectorAll('.listing-card, .toplist-card').forEach(function(card) {
        var name = (card.getAttribute('data-name') || '').toLowerCase();
        var text = card.textContent.toLowerCase();
        card.style.display = (name.indexOf(val) >= 0 || text.indexOf(val) >= 0) ? '' : 'none';
      });
    });
  };
})();
...
function filterNews(btn, cat) {
  ...
  requestAnimationFrame(function() {
    ...
    cards.forEach(function(card) {
      ...
      else if (cat === 'editorial') match = card.getAttribute('data-promo') !== 'yes';
      else if (cat === 'promos') match = card.getAttribute('data-promo') === 'yes';
      else match = (card.getAttribute('data-category') || '').toLowerCase() === cat.toLowerCase();
      if (match) {
        card.style.display = '';
        visible++;
      } else {
...
/* ===== CASINO QUICK FILTERS ===== */
window.casinoFilter = function(btn, type) {
  document.querySelectorAll('.casino-filter').forEach(function(b) { b.classList.remove('active'); });
  btn.classList.add('active');
  var cards = document.querySelectorAll('.toplist-card, .listing-card');
  cards.forEach(function(c) { c.style.display = ''; });
};

// Rating bar + dial scroll animations
(function() {
  function animateRatings() {
    document.querySelectorAll('.rating-bar-fill:not(.animated)').forEach(function(bar) {
      var rect = bar.getBoundingClientRect();
      if (rect.top < window.innerHeight - 50) {
        var targetWidth = bar.getAttribute('data-width') || '';
        if (targetWidth) bar.style.setProperty('--target-width', targetWidth);
        bar.classList.add('animated');
      }
    });
    document.querySelectorAll('.rating-ring-circle:not(.animated)').forEach(function(circle) {
      var rect = circle.getBoundingClientRect();
      if (rect.top < window.innerHeight - 50) {
        var targetOffset = circle.getAttribute('data-offset') || '0';
        circle.style.setProperty('--target-offset', targetOffset + 'px');
  ...
  document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.rating-bar-fill').forEach(function(bar) {
      ...
      if (w) { bar.setAttribute('data-width', w); }
    });
    document.querySelectorAll('.rating-ring-circle').forEach(function(circle) {
      var offset = circle.getAttribute('stroke-dashoffset');
      if (offset) { circle.setAttribute('data-offset', offset); }
    });
    setTimeout(animateRatings, 150);
    window.addEventListener('scroll', animateRatings, { passive: true });
  });
})();
